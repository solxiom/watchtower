...
function runAttachment(signal: NodeJS.Signals): Promise<AttachRun> {
    const laneDir = mkdtempSync(join(tmpdir(), 'watchtower-watch-process-'));
    ...
    return new Promise((resolve, reject) => {
        ...
        const source = `
            ...
            const lane = {
                ...
                kind: 'implementation', controlHome: ${JSON.stringify(laneDir)}, laneDir: ${JSON.stringify(laneDir)},
                lifecycle: 'active',
        ...
        const finish = (result: AttachRun): void => {
        ...
        child.on('close', (status, closeSignal) => finish({status, signal: closeSignal, stdout, stderr}));
        child.on('error', (error) => { clearTimeout(timeout); rmSync(laneDir, {recursive: true, force: true}); reject(error); });
    });
}
