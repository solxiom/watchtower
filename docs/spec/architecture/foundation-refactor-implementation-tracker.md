...
| FR-01 | Baseline dependency architecture gate | ❌ Pending | `spec/foundation/foundationDependencyArchitecture.spec.ts` passes on current tree |
...
| ID | Work unit | State | Acceptance proof |
|----|-----------|-------|------------------|
| FR-03 | `laneStore` facade move | ❌ Pending | `LaneStore.ts` inside `laneStore/`; specs green |
| FR-04 | `transactionalWriter` facade move | ❌ Pending | Facade inside capsule |
...
|-------|----------:|---------:|-------|
| REF-01 | FR-00 … FR-24 | 1 / 25 | ⏳ In progress |
...
| `laneStore/` | L5 | FR-03 | partial | ❌ | Shadow facade at root |
| `transactionalWriter/` | L5 | FR-04 | partial | ❌ | Shadow facade at root |
...
| Spec | FR | State |
|------|-----|-------|
| `foundationDependencyArchitecture.spec.ts` | FR-01, FR-29 | ❌ Not created |
...
| `indexQueryArchitecture.spec.ts` | FR-08, FR-09 | ✅ Exists — update paths in FM-1 |
