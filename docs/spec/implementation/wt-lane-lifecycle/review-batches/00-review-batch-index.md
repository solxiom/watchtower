...
| LC-05 | LC-05 | Shipping-policy baseline: every value matches v1-contracts.md §7 exactly; routing policy: all 16 rules present in correct order; operator-session policy defaults match; provenance markers reference correct spec sections; pack index seal matches active pack seal; deterministic byte-identical reproduction; no model invocation; no full-pack fallback |
