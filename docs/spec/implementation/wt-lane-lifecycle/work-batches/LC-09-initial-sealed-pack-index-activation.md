# Batch LC-09 — Initial sealed pack-index activation
...
Implement only **Initial sealed pack-index activation**. This batch exclusively owns initialization adapter over accepted pack-index compiler. It
...
**Implementation:** Files/interfaces: InitialPackIndexActivation adapter over accepted CA-01 compiler and LC-05 policy roots. Verify active seal/input identity, staged compile, integrity and atomic activation; inject failures before/after compile, verify and rename. Reject JSON alternate authority, duplicate compiler logic, stale seal and partial activation.

## Mandatory proof

- Seal-bound compile/verify/atomic activation; no duplicate compiler or JSON authority.
...
The exclusive owned interface set is **initialization adapter over accepted pack-index compiler**. Before editing, resolve those named owners to exact existing or proposed modules, public symbols, schema/help/task identifiers, and focused specs in the assigned checkout. Record that source-backed mapping in `.local/agent-reports/wt-lane-lifecycle/LC-09-initial-sealed-pack-index-activation.md`. Do not move behavior into a generic helper, a command, a TaskHandler, a mutable registry, workflow shell, or an adjacent batch owner.
...
Do not commit or issue a verdict. Only after every gate passes, write `.local/agent-reports/wt-lane-lifecycle/LC-09-initial-sealed-pack-index-activation.md`, truthfully record this batch's handoff readiness without changing unrelated tracker rows, and emit exactly one replay-safe handoff. A correction retains lineage and reruns both impacted and original acceptance proof.
