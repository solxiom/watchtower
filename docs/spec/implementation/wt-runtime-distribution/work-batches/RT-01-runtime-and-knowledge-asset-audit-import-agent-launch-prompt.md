...
10. `docs/spec/architecture.md` — especially §§4.5 (lane task runtime and leaf
    adapter), 4.6 (knowledge pack)
...
Audit, classify, and import every inherited runtime and knowledge asset with complete
provenance:
...
1. Discover every shell runtime script in the inherited
   ...
   line count, description, coordinator action(s), inputs/outputs,
   mutation/authority assumptions, and external tools for each script. Assign
   exactly one migration class: `replace-with-task-handler`,
   `retain-as-leaf`, `temporary-wrapper`, or `remove`. Temporary wrappers name
   an owning TaskHandler, compatibility reason, removal batch, and expiry.
   Workflow/copy/validation/journal/projection scripts cannot be leaves;
   bounded tmux/Git/agent/tool integrations qualify only when judgment-free.
...
4. Write the audit into `src/foundation/RuntimeAssets.ts` (runtime script
   records) and `src/foundation/AssetAudit.ts` (behavioral inventory).
...
- Do not silently modify inherited content during provenance capture; verbatim
  import does not approve workflow-level shell for distribution.
...
- Do not implement NVB staging, catalog, handlers, or adapters; do provide the
  migration classification those later batches consume.
...
- complete migration classification with no workflow-level shell mislabeled as
  a leaf and no temporary wrapper lacking owner/removal batch/expiry
