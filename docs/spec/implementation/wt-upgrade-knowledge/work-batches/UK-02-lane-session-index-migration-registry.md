# Batch UK-02 — Migration framework and preservation harness
...
Exclusive map ownership: migration registry/staging contracts.

Own a closed MigrationRegistry, ordering/staging interface and preservation harness. Ship no fictional v2/v3 transition; actual format owners register only accepted real transitions and rebuild adapters.
...
5. Independently reproducible acceptance claim: Closed declared transitions only; no fictional versions; capability-owned rebuild adapters; value/history/pin/lifecycle preservation.
...
- Exact implementation report: `.local/agent-reports/wt-upgrade-knowledge/UK-02-lane-session-index-migration-registry.md`
...
The exclusive owned interface set is **migration registry/staging contracts**. Before editing, resolve those named owners to exact existing or proposed modules, public symbols, schema/help/task identifiers, and focused specs in the assigned checkout. Record that source-backed mapping in `.local/agent-reports/wt-upgrade-knowledge/UK-02-lane-session-index-migration-registry.md`. Do not move behavior into a generic helper, a command, a TaskHandler, a mutable registry, workflow shell, or an adjacent batch owner.
...
Do not commit or issue a verdict. Only after every gate passes, write `.local/agent-reports/wt-upgrade-knowledge/UK-02-lane-session-index-migration-registry.md`, truthfully record this batch's handoff readiness without changing unrelated tracker rows, and emit exactly one replay-safe handoff. A correction retains lineage and reruns both impacted and original acceptance proof.
