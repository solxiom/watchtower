...
9. the actual changed source files:
    - `src/foundation/EffectPlan.ts`
    - `src/foundation/EffectExecutor.ts`
    - `spec/basic/effectExecutor.spec.ts`
...
Trace every mutating TaskHandler to the sole executor and one valid single-use
invocation envelope. Reproduce forged, expired, reused, wrong-action/effect,
stale, tampered, and concurrent-double-use rejection; verify no arbitrary task/
target/path/command/environment is present and NVB results are evidence only.
