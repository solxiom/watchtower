...
11. the actual changed source files:
    - `src/foundation/JournalWal.ts`
    - `src/foundation/JournalIndex.ts`
    - `src/foundation/JournalProjection.ts`
    - `spec/basic/journalIndex.spec.ts`
    - `spec/basic/journalProjection.spec.ts`
...
3. **Partial-tail handling**: Create a journal with a truncated final line.
   ...
   indexes only the complete prefix, preserves authoritative JSONL
   byte-for-byte, and keeps mutation blocked pending completion or separately
   authorized repair.
...
9. **No raw SQL outside capsule**: Grep for `.exec(`, `.run(`, `.prepare(`,
   `.all(`, `.get(` — prove they appear ONLY in `JournalWal.ts`.
...
- No raw SQL outside `JournalWal.ts`.
