...
| CA-18 | Accepted OpenTUI evidence promotion and packaging gate | ✅ Accepted | Revalidate TUI-EXP-01 against current exact package/target; no renderer reselection or repeated disposable experiment |
| CA-19 | TUI shell, responsive layout, themes, and focus | ✅ Accepted | Wide right inspector shell; model-free lane entry; P0–P5 attention; standard/narrow layouts; resize; focus/keymap; themes; transactional preferences; bounded animation |
| CA-20 | Conversation timeline, composer, history, and references | ✅ Accepted | Virtualized paging; multiline input; bounded draft recovery; paste; completion; index-bounded timeline search/reference pickers; scroll anchoring; corrections 01–03 verified closed |
| CA-21 | Inspector views, command palette, and overlays | ✅ Accepted | All bounded inspector states; projection-only agent/allocation view; bounded search/attention; canonical action parity; confirmation, diagnostics, and details overlays; correction-04 independently verified |
| CA-22 | Turn streaming, notifications, concurrency, and observer UI | ✅ Accepted | Provisional validation; live edge; stale confirmation invalidation; cross-attachment contention/wait; observer restrictions; priority-preserving coalesced refresh; correction-03 closes CA22-06 post-open attach cancellation and repairs the consolidated-matrix harness (witness-instrumented) on `wt/ca-22` at base `808de693` with `nvb build`/`test` (2964 specs, 0 failures)/focused CA-22 (153 specs, 0 failures)/abort probe/`dist` green; independently accepted in correction-03 re-review |
| CA-23 | Accessibility, terminal lifecycle, recovery, and PTY matrix | ✅ Accepted | Exact promoted matrix; no-color/high-contrast/reduced motion; signals/suspend/crash restore; preference/cache migration; semantic visual catalog; emulator/Unicode/resize fixtures |
| CA-24 | Session command integration, specification-resolution proof, scale/replay, and M6 acceptance | ✅ Accepted | Independent R5 review on `wt-review-CA-24-kavan2`: packaged coordinator/session/ask dispatch, complete contradiction→advice→authority→re-seal→activation→explicit sync→same-session resume fixture, 30/300/3,000/10,000-batch scale invariance, long-session replay, `nvb build`/`dist`, and full `nvb test` (3130 specs, 0 failures) all green; corrections 01–02 closed |
| CA-25 | Cycle, escalation, and specification-resolution commands | ✅ Accepted | Cycle/escalate/resolution dry-run purity; normal validator/executor only; no command-local authority; integrated at 46911e1 handoff 20260809T232658Z-1724650-7016 |
...
| CA-30 | Pack-index build and runtime-index rebuild command | ✅ Accepted | `index build [--runtime]`; dry-run purity; staged compile/rebuild; current-state validation; no command-local mutation |
| CA-31 | Coordinator, session, and TUI doctor providers | ✅ Accepted | Coordinator/session/TUI checks; exact pass/warn/fail/skip; read-only; release only qualifies behavior; correction-01 independently re-reviewed |
