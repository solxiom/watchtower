...
| CA-05 | Ordered routing policy and capability floors | ✅ Accepted | Every v1 rule/guard; first-match determinism; installed-policy provenance; normative contradiction after safety as D3/C5; economics only after hard eligibility |
...
| CA-12 | Acceptance and Git publication handler | ❌ Pending | Reviewer-session ownership; commit-set validation; partial push recovery; Nirvana Git API audit |
...
| CA-15 | Operator-session persistence and lifecycle | ❌ Pending | Many sessions; one active turn each; immutable closed history; crash-safe journals |
| CA-16 | Session SQLite index, references, pins, and compaction | ❌ Pending | Bounded metadata/excerpts; exact text remains journal-owned; same-lane capsules; no full-history fallback |
...
| CA-28 | OpenCode decision-endpoint adapter | ❌ Pending | Required unattended conformance; bounded argv/env/cwd/result; fresh catalog/model fingerprint |
| CA-29 | Hermes decision-endpoint adapter | ❌ Pending | Same conformance when installed; explicit healthy `not-installed` outcome |
