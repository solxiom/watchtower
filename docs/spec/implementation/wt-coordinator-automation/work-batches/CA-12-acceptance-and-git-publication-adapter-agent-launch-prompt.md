...
10. the canonical source owners you will actually work with:
    - `src/foundation/GitAcceptance.ts` (create)
...
0. Audit pinned Nirvana Git APIs and comparable Nirvana/Nira usage for object/
   ref/tree/ancestry inspection, push, typed results, credentials/environment,
   and cancellation. Implement a focused packaged TaskHandler with a valid
   CA-10 single-use envelope. Prefer a conforming Nirvana Git API; otherwise
   record `NIRVANA_API_GAP` and use one cataloged bounded Git leaf through
   `LeafRuntimeInvoker`. No caller-controlled remote/refspec/config/path/
   executable/environment or raw shell is accepted.

1. Create `src/foundation/GitAcceptance.ts` with `GitAcceptanceAdapter`,
...
- Do not add npm scripts, public/project-root tasks, or unrelated catalog
  tasks. This batch must implement its focused packaged acceptance/publication
  TaskHandler and audited Git API/leaf through the accepted runtime-nvb surface.
- Do not commit.
