...
Accept only when every applicable discovery/selection precedence and ambiguity
cell has independent proof, invalid candidates fail closed, candidate output is
actionable, canonical path behavior is reused rather than duplicated, all reads
produce zero writes, and every engineering-standard matrix gate passes.

## Rejection Correction Brief Rule

Write a numbered RM-06 correction under `review-batches/corrections/` including
the complete failing matrix row, fixture layout, expected result, actual result,
...
Update tracker, roadmap, and indexes only after matrix and filesystem-hash proof.
Never infer acceptance from one happy path.

## Local Artifact Git Rule

Keep temporary workspaces, symlinks, hashes, and reports local. Commit only
bounded sanitized fixtures intentionally owned by tests.

## Non-Negotiable Rules

One selector owns precedence. Commands do not reimplement discovery, ambiguity
never chooses arbitrarily, RM-07 retains secondary-discovery ownership, and
read paths never repair state.

## Required Disk Report

`.local/agent-reports/wt-read-model/reviews/RM-06-home-lane-discovery-and-selection-review.md`

Include the complete matrix, fixture inventory, before/after hashes, reviewer
...
Use the matrix itself as the review task list.

## Leave a helpful handoff message for the next reviewer

Record precedence, candidate/error shapes, excluded RM-07 behavior, report
path, and acceptance/correction identity.
