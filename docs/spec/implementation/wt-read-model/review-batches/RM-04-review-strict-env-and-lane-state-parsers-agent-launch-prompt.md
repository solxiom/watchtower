...
Accept only when the complete accepted scalar and lane-state grammars
round-trip, every executable/shell construct fails closed without execution,
unknown-key preservation and redaction match contract, parser ownership stays
separate, and every engineering-standard matrix gate passes.

## Rejection Correction Brief Rule

Write a numbered RM-04 correction under `review-batches/corrections/` with the
exact malicious bytes, parser state, expected failure, actual result, owner,
...
Update tracker, roadmap, and indexes only after current evidence. Preserve the
normative distinction between strict TypeScript parsing and narrowly declared
legacy leaf compatibility.

## Local Artifact Git Rule

Keep malicious fixtures and reports local unless a sanitized regression fixture
is intentionally test-owned. Never commit secrets or generated artifacts.

## Non-Negotiable Rules

Never call `source`, `eval`, a shell, variable expansion, command substitution,
or ambient environment interpolation. Parse external bytes as unknown.

## Required Disk Report

`.local/agent-reports/wt-read-model/reviews/RM-04-strict-env-and-lane-state-parsers-review.md`

Include grammar tables, malicious corpus, no-execution proof, unknown-key and
redaction cases, reviewer matrix, line counts, commands, and verdict.

## If accepted, commit: `RM-04: Strict env and lane-state parsers accepted`

## Always plan and make task lists

Track every grammar production and malicious class separately.

## Leave a helpful handoff message for the next reviewer

Record accepted grammar, rejected constructs, redaction behavior, report path,
and acceptance/correction identity.
