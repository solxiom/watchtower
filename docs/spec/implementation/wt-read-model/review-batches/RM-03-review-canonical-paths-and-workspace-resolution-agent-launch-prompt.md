...
Accept only when precedence, canonicalization, containment, symlink/case
behavior, explicit-workspace failure, XDG resolution, and zero-write resolution
are independently proved. Confirm RM-03 either proves a CLI-safe
multi-workspace storage-facade bootstrap or records the precise
`NIRVANA_API_GAP`; it must not casually initialize a global storage singleton.
Every engineering-standard matrix gate must pass.

## Rejection Correction Brief Rule

Write a numbered RM-03 correction under `review-batches/corrections/` naming
the escaped/canonical path, trust boundary, expected result, owning module, and
regression fixture. Do not repair production code as reviewer.
...
Update tracker, roadmap, and indexes only after current proof. Do not infer
acceptance from the implementation report.

## Local Artifact Git Rule

Keep temporary workspaces, symlink fixtures, hashes, and reports local. Never
commit build, dist, dependency, or lane-runtime artifacts.

## Non-Negotiable Rules

No hidden mkdir/repair, no comparison before canonicalization, no
worker-controlled log/storage root, and no selection policy inside path
utilities.
...
Include the reviewer matrix, Nirvana/storage audit, precedence table, attack
corpus, before/after filesystem proof, line counts, and verdict.
...
Close each precedence and escape-matrix cell explicitly.

## Leave a helpful handoff message for the next reviewer

Record canonicalization strategy, storage-bootstrap decision/API gap, error
categories, proof paths, and acceptance/correction identity.
