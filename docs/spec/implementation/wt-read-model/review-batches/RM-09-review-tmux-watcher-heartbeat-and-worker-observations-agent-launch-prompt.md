...
1. **Tmux session reading**: Verify fixed argv
   `tmux list-sessions -F '#{session_name}'` crosses the pinned Nirvana command
   facade with explicit cwd, sanitized environment, timeout, and output bound.
   Confirm the command port is injectable for tests and prove there is no
   shell/direct child-process/arbitrary-operation path. Test zero, one, and
   multiple sessions.
2. **Qualified-name identity**: Prove only an exact, validated, fully
   constructed session name matches. Prefix-only, adjacent-lane, case-variant,
   suffix, and Unicode-confusable names must not match.
3. **Heartbeat staleness**: Create heartbeat files with various timestamps.
...
4. **Worker-event reading**: Verify the latest valid worker event per role is
   surfaced correctly from the parsed JSONL stream. Confirm the RM-05 parser
   is consumed through its public interface.
5. **No-mutation proof**: Before running any observation function, compute a
...
6. **No lifecycle authority**: Audit every observation function. Verify none
...
- Tmux session reading crosses the pinned Nirvana command port with fixed argv
  and bounded execution; exact qualified-name matching rejects prefix collisions.
