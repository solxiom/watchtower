...
7. **Dedicated worktree isolation:** Create a dedicated Git worktree. Initialize Lane C on it. Verify success. Run `wt status` and `wt doctor` on Lane A; verify both succeed and Lane A's status does not falsely report a writable-path collision. Cross-worktree same-repository related-lane **warning** is deferred per the amended work batch (Phase 4b); accept source-backed negative proof documenting `SAME_REPOSITORY_DIFFERENT_WORKTREE_WARNING_GAP`.
...
9. **Partial push recovery:** Set up a scenario where one repo's push fails. Verify: succeeded repo's push journal records success (`verified`), failed repo's push journal records failure with rejection reason (`uncertain`/`failed` and typed outcome), `partialRecovery` is true, the `accept` event remains authoritative. Fix the push block and retry. Verify the retry uses the existing acceptance commit (no new acceptance), primary's original journal record is unchanged, and the failed repo's push journal updates to `verified` at a later sequence. `wt status` partial-push **warning** is deferred per the amended work batch (Phase 5b); accept source-backed negative proof documenting `WT_STATUS_PUSH_JOURNAL_WARNING_GAP`.
