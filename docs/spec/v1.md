...
| `wt events tail|latest` | ⏳ | Read validated durable event projections; tail accepts `--since=<event-id>` and latest accepts `--batch=<id>`, without cursor advancement |
| `wt batch ready` | ⏳ | Return a closed, typed ready-set projection (all fields required; unknown fields rejected) without selecting a winner |
