...
*(LC-07, delivered)* below — invocable today as `wt doctor [--workspace=<path>] [--lane=<slug-or-uuid>]
[--json] [--no-color]`. Every other check bullet, `--tui`, and `--report=<path>` are defined here
for the target contract but are not yet implemented or accepted; they are delivered by later batches (at least
LC-10 for the tool/account/runtime/watcher/index checks, and a TUI-delivery batch for `--tui`/`--report`) and
are not currently accepted CLI flags. `wt doctor --help` and `help/commands/doctor.hlp.json` describe only the
currently delivered syntax.
