...
| `wt doctor [--lane=<slug-or-uuid>] [--tui] [--report=<path>]` | ❌ | Validate bindings, conflicts, runtime, tools, accounts, pack, and optional TUI stack; explicitly write a redacted support report. Ships incrementally: LC-07 delivers the read-only kernel/command and marker/config/binding/permission/Git-ignore checks now (see §11.7); the remaining checks and `--tui`/`--report` follow in later batches |
...
below (not that bullet's coordinator-policy, journal, or cursor clauses, which remain undelivered). None of
this is invocable through a new flag — it is exposed automatically as part of the existing
`wt doctor [--workspace=<path>] [--lane=<slug-or-uuid>] [--json] [--no-color] [--verbose]` syntax. Every other
check bullet, `--tui`, and `--report=<path>` are defined here for the target contract but are not yet
implemented or accepted; they are delivered by later batches (at least a TUI-delivery batch for
`--tui`/`--report`) and are not currently accepted CLI flags. `wt doctor --help` and
`help/commands/doctor.hlp.json` describe only the currently delivered syntax.
